...
#   NRANKS         – number of MPI ranks to use (default: 8)
...
#   - Run create_moho_topography.py and convert_s362ani_to_radius.py first
#     to generate the NetCDF files in input_forward/
#
...
# Check that the required NetCDF files exist
for nc_file in moho_topography.nc S362ANI_radius.nc; do
    if [[ ! -f "$INPUT_DIR/$nc_file" ]]; then
        echo "ERROR: required NetCDF file not found: $INPUT_DIR/$nc_file"
        echo "       Run the preprocessing scripts first:"
        echo "         python create_moho_topography.py"
        echo "         python convert_s362ani_to_radius.py"
        exit 1
    fi
done
...
cp -rL "${INPUT_DIR}/"* "${SIMU_DIR}/input/"
